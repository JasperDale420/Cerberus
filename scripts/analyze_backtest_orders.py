...
def analyze_orders():
    ...
    df = pd.read_sql_query(
        "SELECT symbol, side, qty, type, limit_price, status, time_placed, time_last_update, meta_json FROM orders WHERE status = 'filled'",
        conn,
    )
    ...
    df["cash_flow"] = df.apply(
        lambda row: (-1 * row["qty"] * row["limit_price"])
        if row["side"] == "buy"
        else (row["qty"] * row["limit_price"]),
        axis=1,
    )
