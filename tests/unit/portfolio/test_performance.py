...
from src.portfolio.performance import CorrelationAlert, PortfolioPerformance, StrategyAttribution
...
class TestStrategyAttribution:
    ...
    def test_attribution_returns_correct_type(self, perf: PortfolioPerformance):
        strategy_pnls = {"typed": [1.0, 2.0, 3.0]}
        attrs = perf.compute_attribution(strategy_pnls)
        assert isinstance(attrs[0], StrategyAttribution)


class TestSharpe:
    """Test Sharpe ratio computation."""

    def test_sharpe_constant_returns_is_none(self, perf: PortfolioPerformance):
        returns = [0.01] * 60
...
class TestCorrelation:
    ...
    def test_correlation_alerts_fire_above_threshold(self):
        ...
        assert len(alerts) > 0
        assert any(isinstance(a, CorrelationAlert) and a.correlation > 0.5 for a in alerts)
