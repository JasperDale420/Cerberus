...
def _should_start_alpaca_stream(order_executor: str, data_backend: str) -> bool:
    """Return True when Alpaca streaming is required for market data."""
    if data_backend == "gateway":
        return order_executor == "alpaca"
    return True


def _should_initialize_alpaca_client(order_executor: str, data_backend: str, *, failover_to_legacy: bool) -> bool:
    """Return True when Alpaca client is required for execution or fallback."""
    if data_backend == "gateway" and order_executor == "noop" and not failover_to_legacy:
        return False
    return True


def _next_market_open_local(now: datetime) -> datetime:
    """Compute the next regular market open time in the same local tz."""
    tz = now.tzinfo or ZoneInfo("America/New_York")
    open_time = datetime.combine(now.date(), time_type(9, 30), tzinfo=tz)
    ...
    return datetime.combine(next_day, time_type(9, 30), tzinfo=tz)


def _is_regular_market_session_local(now: datetime) -> bool:
    """Return True during regular trading hours in the provided tz."""
    if now.weekday() >= 5:
        return False
    tz = now.tzinfo or ZoneInfo("America/New_York")
    open_time = datetime.combine(now.date(), time_type(9, 30), tzinfo=tz)
    close_time = datetime.combine(now.date(), time_type(16, 0), tzinfo=tz)
