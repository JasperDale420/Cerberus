...
class MockOrderExecutor:
    ...
    async def submit_order(self, intent: OrderIntent) -> Optional[Dict]:
        """
        Simulates order submission.
        """
        self.logger.info("Mock submitting order", symbol=intent.symbol, side=intent.side, qty=intent.qty)
    ...
    def fill_orders(self, symbol: str, price: float, timestamp: str):
        """
        ...
        for order in self.orders:
            if order["symbol"] == symbol and order["status"] == "new":
                ...
                self.logger.info("Mock order filled", symbol=symbol, side=order["side"], price=price)
