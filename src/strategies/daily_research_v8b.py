"""Consecutive Down + IBS + BB mean reversion — high trade volume variant.

Entry requires ALL of:
  1. N+ consecutive lower closes (optimized 2-3)
  2. Low IBS (closed near day's low, optimized 0.20-0.35)
  3. Close within 1.5 std devs of lower BB (wider = more trades)

More trades per window = more statistical significance = less variance.
Skip SHOCK vol, earnings, FOMC.
Target = min(BB midline, ATR target). Short hold.
"""
...
class SeedTrendPullbackStrategy(BaseStrategy):
    ...
    def _set_params(self, config: Dict[str, Any]) -> None:
        ...
        # Wider BB proximity for more trades per window
        self.bb_proximity = 1.5
    ...
    def on_bar(
        ...
        # Required: Close near lower BB (wide proximity for more trades)
