...
iter11: Tighter symmetric 1.5x/1.5x ATR stops (was 2x/2x).
- Tighter stops = more trades resolved by stop/target, fewer time-exits
- Price-based trend filter: close > SMA(20)
- RSI(2) < 25 normal, RSI(2) < 10 in high-vol (ATR ratio only)
- 1.5x ATR stop, 1.5x ATR target (capped at 4%)
- 12% drawdown filter
