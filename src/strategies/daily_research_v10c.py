...
class SeedVolBreakoutStrategy(BaseStrategy):
    ...
    def on_bar(
        ...
        regime_vol = labels.get("vol_regime_symbol", "NORMAL")
        if regime_vol in ("SHOCK", "HIGH"):
            return None

        regime_trend = labels.get("trend_regime_symbol", "FLAT")
        ...
        # Moderate compression: map atr_dip_min [0.3, 1.0] → effective [0.3, 0.8]
        # Reduces sensitivity without killing trade count
        effective_mult = 0.3 + (self.atr_dip_min - 0.3) * (0.50 / 0.70)
