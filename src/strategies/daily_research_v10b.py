"""Trend Pullback with BB Width Guard — Consec Down + IBS in Stable Uptrends.

Entry: SMA(10) > SMA(50) uptrend + close > SMA(20) + BB width below threshold
...
class SeedTrendPullbackStrategy(BaseStrategy):
    ...
    def on_bar(
        ...
        # --- Trend: SMA(10) > SMA(50), close > SMA(20) ---
        sma_f = self._sma(closes, self.sma_fast)
        sma_m = self._sma(closes, self.sma_mid)
        ...
        # --- BB width guard: skip choppy/volatile markets ---
        bb_std = self._std(closes, self.sma_mid)
        if bb_std is None or sma_m < 1e-9:
            return None
        bb_width = (2.0 * bb_std) / sma_m
