"""IBS Trend Pullback v3 — SMA(50) filter + skip DOWN+HIGH.

Double regime filter for maximum consistency:
  1. Close > SMA(50) — stock-level uptrend
  2. Skip DOWN+HIGH vol — market-level regime gate
IBS < threshold for oversold signal (wide range).
1 down day required. Wider stop (2.0 ATR) with max_stop_pct cap.
...
class dailyresearchv7dStrategy(BaseStrategy):
    ...
    def on_bar(
        ...
        # Skip DOWN+HIGH vol combo (market-level)
        ...
        # Stop: ATR-based but capped at max_stop_pct
