...
class GapFillStrategy(BaseStrategy):
    ...
    def on_bar(
        ...
        if float(self.min_or_range_pct) > 0:
            if open_price <= 0:
                self.logger.warning(
                    "GapFill: Opening range open price invalid",
                    symbol=symbol,
                    open_price=open_price,
                )
                return None
            or_range_pct = (float(or_high) - float(or_low)) / open_price
            if or_range_pct < float(self.min_or_range_pct):
                self.logger.debug(
                    "GapFill: Opening range too tight",
                    symbol=symbol,
                    or_range_pct=round(or_range_pct * 100, 4),
                    min_or_range_pct=round(float(self.min_or_range_pct) * 100, 4),
                )
                return None
