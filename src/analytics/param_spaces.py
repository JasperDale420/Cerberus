...
PARAM_SPACES: dict[str, list[ParamDef]] = {
    ...
    # v7 seed archetypes
    "daily_research_v7a": [
        ParamDef("ibs_threshold", "float", low=0.25, high=0.45, step=0.05, description="IBS entry threshold"),
        ParamDef("stop_atr_mult", "float", low=1.0, high=1.5, step=0.5, description="ATR stop multiplier"),
        ParamDef("target_atr_mult", "float", low=1.5, high=2.5, step=0.5, description="ATR target multiplier"),
        ParamDef("vol_mult", "float", low=0.5, high=0.8, step=0.1, description="Volume filter multiplier"),
    ],
    "daily_research_v7b": [
        ParamDef("consec_down_days", "int", low=2, high=3, step=1, description="Consecutive down days"),
        ParamDef("ibs_entry_threshold", "float", low=0.2, high=0.4, step=0.05, description="IBS entry threshold"),
        ParamDef("max_atr_pct", "float", low=0.025, high=0.05, step=0.005, description="Max ATR/price vol filter"),
    ],
    "daily_research_v7c": [
