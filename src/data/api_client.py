...
class CentralApiClient:
    ...
    def __init__(self, config_loader: ConfigLoader, logger: StructuredLogger):
        self.logger = logger
        self.base_url = config_loader.get_env("DATA_INGESTION_URL", "http://localhost:8000")
        self.client = httpx.Client(base_url=self.base_url, timeout=30.0)

    def get_alpaca_bars(self, symbol: str, start: Optional[datetime] = None, end: Optional[datetime] = None, timeframe: str = "1Day") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.error("Failed to fetch Alpaca bars from central API", symbol=symbol, error=str(e))
            raise
