...
The new `regime_flat` block added on 2026-05-04 (above) was authored before this change merged; its `risk:` line was stripped during the merge to align with this policy.
...
- **[CRITICAL C1+C2] `ratio_vs_spy` math repaired.** Two compounding errors silently broke the headline metric:
  - **Sign flip in bear markets.** `ratio = strat_pct / spy_pct` reported a strategy losing -20% in a -10% SPY window as `ratio=2.0` (passing the 2x SPY headline goal while LOSING money). Conversely, +5% in a -10% SPY window reported `ratio=-0.5` (failing the gate despite genuine outperformance).
  - **Apples-to-oranges return calculation.** Strategy used `sum(net_pnl) / $100k` (additive, fresh capital each window); SPY compounded per-window growth factors. With identical 5%/window returns, strategy reported 90%, SPY reported 140.7%, ratio=0.64 — strategy looked 36% worse despite identical performance.
  - **Fix:** Strategy returns are now compounded per-window (`running_capital *= 1 + window_pnl/running_capital`, floored at -99% per window). The ratio is computed in three sign-safe modes: bull (`SPY > +1%`: standard ratio), bear (`SPY < -1%`: `1 + alpha/|spy|` so breaking even in a -10% drawdown reports as 2.0), flat (`|SPY| <= 1%`: absolute strategy return as alpha proxy — fixes the FLAT-specialist unscoreable case from M6). `AUTORESEARCH_BENCHMARK` now emits `mode=` so the agent can see which formula was applied.
- **[HIGH H1] Driver no longer crashes when agent edits a protected file.** L220 referenced `$EVAL_STRATEGY` (assigned later at L272) inside the protected-files violation TSV row. With `set -uo pipefail`, this killed the entire driver session — the defense-in-depth layer was a bomb. Fixed to use `STRAT_FILE` (in scope from L125).
- **[HIGH H2] Phase pivot now resets BEST_SCORE/BEST_COMMIT.** Previously the pivot advanced `CURRENT_PHASE` and `STRAT_FILE` but left BEST_SCORE/BEST_COMMIT carrying state from the previous phase — new-phase strategies were compared against old-phase scores (incoherent), and discards reset to old-phase code. The bootstrap branch (`BEST_SCORE <= -999`) was dead code post-baseline. Now: pivot writes `BEST_SCORE=-999.0` and `BEST_COMMIT=$BASELINE_COMMIT`, reviving the bootstrap branch for the first iter of the new phase.
- **[HIGH H3] Baseline strategy aligned with first phase target.** The driver's `$1` arg was used for baseline (default `rsi_bounce`) but iterations always used `REGIME_PHASES[CURRENT_PHASE]`. Live log showed `Strategy: regime_adaptive` baseline followed by iter 1+ targeting `regime_trend_up` — two different strategies under one BEST_SCORE comparison. Now: `$1` (renamed `START_PHASE_ARG`) resolves to a phase index by name; `STRATEGY` is derived from `CURRENT_PHASE` so baseline runs against the same strategy iter 1+ target. Unknown `$1` falls back to phase 0.
- **[MEDIUM M1] `--target-regime` no-match is now an explicit gate failure.** Previously `if target_regime and regime_stats.get(target_regime):` silently fell open to all-windows when zero windows classified as the target regime — a regime specialist got scored against all 18 windows, guaranteeing gate failure. The FLAT+NORMAL phase (5% of data) was most exposed. Now: filter is attempted unconditionally; zero-match emits `REGIME_FILTER_NOMATCH` and adds `regime_filter_no_match=<target>` to the gate failures.
- **[MEDIUM M2] `claude -p` agent calls wrapped in `timeout`.** Eval calls already had `timeout 10800`; agent calls had no bound, so a hung session would block the loop indefinitely. Main agent: 1800s. Import-fix agent: 600s.
- **[MEDIUM M3] 4 legacy scorer scripts replaced with deprecation stubs.** `scripts/autoresearch_score.py`, `autoresearch_score_cv.py`, `autoresearch_pairs_score.py`, `autoresearch_trp_sortino.py` all used the gameable pre-2026-04-25 weighted-average formula (`0.30*pnl + 0.25*sharpe + ...`) replaced by `ratio_vs_spy` in `cerberus_autoresearch.py`. They had no callers in `src/` but remained runnable footguns. Each now exits non-zero (code 2) with a deprecation message pointing to the active scorer. Total: -481 LOC.
- **[MEDIUM M4] Driver auto-config uses full activation set.** The auto-appender used `session: [opening, midday, power_hour]` and `vol: [low, normal, high]` — silently locking new strategies out of premarket/close/shock conditions despite the agent prompt's own "Keep activation permissive" instruction. Now uses the full set already in `config/strategies.yaml`: `session: [premarket, opening, midday, power_hour, close]`, `vol: [low, normal, high, shock]`.
- **[MEDIUM M5] Playbook holdout claim corrected; new scoring math documented.** `program_cerberus.md` advertised "3-month final holdout the agent never sees during iteration" as an anti-overfit guard, but the harness only excludes the date range from training/OOS — no validation runs against it. Updated playbook to honestly describe the holdout as a date-range exclusion only (implementation flagged as future work) and to document the post-fix bull/bear/flat scoring modes, the new `regime_filter_no_match` gate, and the multiplicative `loc_multiplier`.
- **[MEDIUM M7] HISTORY block uses `tail -3` not `tail -4 | head -3`.** The agent prompt's HISTORY iterated rows N-4..N-2 instead of N-3..N-1, dropping the most recent prior iteration. Agent could repeat iter N-1's failed approach because it wasn't in the "don't repeat" history block.
- **[LOW L1] PROTECTED_FILES extended.** Added `scripts/autoresearch_loop.sh` and `program_cerberus_v3.md` to the file-restoration set. Harmless under v2 driver; protects them if the v3 path runs in the same workspace.
- **[LOW L2] Commit message sanitization strips tabs.** TSV is tab-delimited; an agent commit message with a tab character would corrupt the description column and break HISTORY parsing. `tr -d '\n\r'` extended to `tr -d '\n\r\t'`.
...
- **[CRITICAL] `BacktestReportCard.to_dict()` now exposes the per-trade list.** Previously the `trades` key was missing from the serialized report; downstream `oos_metrics["trades"]` was always `[]`. This made `compute_regime_diversity_multiplier` always short-circuit to `multiplier=1.0` (the regime-diversity anti-overfit guard advertised in `program_cerberus.md` was inert) and forced `extract_wfo_insights.py` to use the aggregate-only fallback even when 18,000+ trades existed. New `_trade_to_dict()` helper serializes each `TradeRecord` (dataclass) and surfaces `exit_reason` from `meta`, `hold_minutes` from `(exit_time-entry_time)`, plus the existing regime-context fields.
- **[HIGH] SPY benchmark now compounds per-window returns instead of using span min..max.** When `--target-regime` filtered windows to a non-contiguous subset, the SPY benchmark spanned the full date range *including* gap windows the strategy didn't trade — making `ratio_vs_spy` apples-to-oranges. Now: `spy_total_return *= seg.iloc[-1].close / seg.iloc[0].close` per window, then `(spy_total_return - 1) * 100`. `benchmark_span` reports `<N> segments <min_date>..<max_date>` so the user can see compounding is at play.
- **[HIGH] Driver bootstrap requires `composite_score > GATE_FLOOR` (-2.0).** Previously the first iteration with >30 trades was kept regardless of harness gates, enshrining a known-broken strategy as the lineage baseline. New `GATE_FLOOR_LOCAL=-2.0` check prevents gate-failed iterations from becoming `BEST_COMMIT`.
- **[MEDIUM] Driver verifies the agent actually edited the target strategy file before evaluating.** A `git diff "$BEST_COMMIT" HEAD --name-only | grep -q "^src/strategies/${STRAT_FILE}\.py$"` guard fires before the eval; no-op or wrong-file commits skip the eval and record a `no_strategy_change` row in the TSV. Previously these wasted ~30-75 minutes per occurrence.
- **[MEDIUM] Stale `<strategy>_latest.json` no longer leaks across iterations.** Two-layer defense: (1) driver `rm -f`s the JSON immediately before each eval; (2) harness stamps `meta.commit_sha` + `meta.eval_completed` + `meta.wfo_window` into the saved JSON, and `extract_wfo_insights.py` bails with `STALE_INSIGHTS` if `meta.commit_sha != HEAD`. Crashed evals can no longer feed phantom trade analysis to the next agent.
- **[MEDIUM] `composite_score` no longer mixes units with `loc_penalty`.** Old: `composite = ratio_vs_spy + loc_penalty`, which inflated a 0.01 ratio + 0.5 LOC bonus to a misleading 0.51. New: `composite = ratio_vs_spy * (1 + loc_penalty * 0.05)` — multiplicative, ±10% effective range. Also tightened the small-SPY guard from `if spy_return_pct != 0` to `if abs(spy_return_pct) > 1.0` so floating-point SPY noise can't blow up the ratio.
- **[LOW] Trial DB sweeper at driver startup.** `find .agents/tmp/optuna_dbs/ -name 'trial_*.db*' -mmin +60 -delete` reclaims the 458 MB / 436 files of stale trial DBs left behind by SIGKILL'd Optuna workers since Apr 15.

### Added

- **Autoresearch driver hardening — `BASELINE_COMMIT` + `BEST_COMMIT` pointers, protected-file restore.** Discards now `git reset --hard $BEST_COMMIT` (last known-good iteration) instead of `HEAD~1`. This prevents the driver from walking back through infrastructure commits (harness fixes, playbook updates, baseline reverts) when the agent makes unrelated commits or when discards accumulate. Added a pre-iteration `restore_protected` step that re-checkouts `scripts/cerberus_autoresearch.py`, `scripts/extract_wfo_insights.py`, `scripts/autoresearch_driver.sh`, and `program_cerberus.md` from `BASELINE_COMMIT` so each eval runs against a clean harness. Added a post-agent violation check that aborts the iteration if the agent touched anything outside the strategy sandbox. Root cause: a previous run's `git reset --hard HEAD~1` walked the harness back to its pre-honest-scoring state mid-run, producing a series of "best score" iterations that were measured by the lying old scorer (ratio_vs_spy=-1.56 reported as composite=4.9458).
...
- **Honest autoresearch scoring** — composite score is now `ratio_vs_spy` directly (with LOC/CV adjustments) instead of "average of OOS scores after filtering hard-rejects". Three hard gates force the score to `-2.0` on failure: `windows_profitable_pct ≥ 40%` (must actually make money on a plurality of OOS windows), `ratio_vs_spy > 0` (must be net profitable over the span), and `total_oos_trades ≥ 5×n_windows` (minimum activity). The previous scoring let a strategy that lost $333k while SPY tripled register a "best score" of 1.84 by averaging the one lucky window — that loophole is closed.
- **`extract_wfo_insights.py` aggregate fallback** — when per-trade detail is missing from `oos_metrics["trades"]` but `n_trades > 0` aggregates exist, emit a window-level summary (PnL/PF/WR/Sharpe per window) instead of falsely reporting `NO_TRADES`.
...
- **Regime context fields on TradeRecord and trade reconstruction**: `TradeRecord` now carries explicit regime context fields at both entry and exit: `entry_regime_trend`, `entry_regime_vol`, `entry_liquidity`, `entry_correlation`, `entry_near_earnings`, `entry_near_fomc`, `entry_opex_week`, `entry_session_phase`, `exit_regime_trend`, and `exit_regime_vol`. These are populated from two sources: (1) signal-level regime tags stored in `OrderIntent.meta` and persisted in `DbOrder.meta_json`, and (2) pre-computed parquet regime labels as fallback. The `BacktestAnalyzer` FIFO matching in `stats.py` also carries regime labels through fills into trade dicts. All fields default to "unknown"/False for backward compatibility.
...
- **Regime label loading and propagation in backtest runner**: The backtest runner now loads pre-computed daily regime labels from `data/regime_labeled/{SYMBOL}_daily_regime.parquet` and 1-minute session phase labels from `data/regime_labeled/{SYMBOL}_1m_session.parquet`. Regime context (trend, volatility, liquidity, earnings/FOMC/OpEx windows, correlation regime, SPY beta) is attached to `symbol_state.meta["regime_labels"]` on each bar, making it accessible to strategies and the execution engine during backtests. Session phase is attached to `symbol_state.meta["session_phase"]`. Regime labels are also captured on each `TradeRecord.meta["regime_labels"]` at entry time, enabling regime-aware post-trade analysis. The diagnostics engine now uses actual regime trend labels instead of hardcoded "UNKNOWN". All regime data is optional — backtests work exactly as before when labeled data is absent.
...
- **Exit efficiency wrong for short trades**: `compute_entry_exit_efficiency` computed `pnl_from_entry` as `exit_price/entry_price - 1` (long-only formula). Profitable shorts showed negative PnL, clamped to 0% exit efficiency. Now uses direction-adjusted `pnl_r` directly.
...
- **GPD tail index (xi) estimator sign inverted — underestimates heavy tail risk**: Method-of-moments formula computed `xi = 0.5 * (mean²/var - 1)` but the correct GPD formula is `xi = 0.5 * (1 - mean²/var)`. The sign inversion classified heavy-tailed distributions (true xi > 0) as light/bounded tails (xi < 0), systematically underestimating tail risk and allowing larger positions during exactly the conditions that warrant maximum caution.
